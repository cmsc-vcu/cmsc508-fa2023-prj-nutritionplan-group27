...
1. /users/username: This route takes in the standard arguments as well as the header 'search' which is a string. If search is not given the api returns all users sorted by their search. If search is given then it returns all users with usernames similar to that string
